...
def export_bank_reconciliation(conciliacion_edo_cta_sap: pd.DataFrame, output_file: str,edo_cta_cols: list[str]):
    ...
    with pd.ExcelWriter(output_file, engine='xlsxwriter', datetime_format='dd-mm-yyyy') as writer:
        ...
        df_diff_importe_dsp.to_excel(writer, sheet_name='Diferencia importe 1MXN/01USD', index=False, freeze_panes=(2, 0), startrow=1)
        ...
        writer.sheets['Diferencia importe 1MXN/01USD'].set_tab_color('#FFE699') # amarillo claro
        ...
        for sheet in ['Bancos vs SAP', 'Conciliados', 'No conciliados', 'Diferencia importe', 'Diferencia fecha', 'Resumen', 'Totales por moneda', 'Diferencia importe 1MXN/01USD']:
            worksheet = writer.sheets[sheet]
            # Determina columnas a formatear
            if sheet in ['Bancos vs SAP', 'Conciliados', 'No conciliados', 'Diferencia importe', 'Diferencia fecha', 'Diferencia importe 1MXN/01USD']:
                df = df_full
            elif sheet == 'Resumen':
                df = resumen
            else:
...
def export_sap_reconciliation(conciliacion_sap_vs_edo: pd.DataFrame, output_file: str, sap_cols: list[str]):
    ...
    with pd.ExcelWriter(output_file, engine='xlsxwriter', datetime_format='dd-mm-yyyy') as writer:
        ...
        df_diff_importe_dsp_sap.to_excel(writer, sheet_name='Diferencia importe 1MXN/01USD', index=False, freeze_panes=(2, 0), startrow=1)
        ...
        writer.sheets['Diferencia importe 1MXN/01USD'].set_tab_color('#FFE699')
        ...
        for sheet, df in [
            ...
            ('Diferencia importe 1MXN/01USD', df_diff_importe_dsp_sap),
